...
Usage:
    python3 executors/ideas/google_trends_ideas.py \
...
def main():
    ...
    parser.add_argument("--max-keywords", type=int, default=15,
                        help="Max keywords to analyze (randomly sampled if more; default: 15)")
    ...
    # Sample keywords if more than max
    import random
    if len(keywords) > args.max_keywords:
        keywords = random.sample(keywords, args.max_keywords)
    ...
    if args.output:
        ...
        rising_count = sum(1 for kt in trends_data["keyword_trends"]
                          if kt.get("trend_direction") == "rising")
        print(json.dumps({
            "status": "success",
            "output": args.output,
            "keywords_analyzed": len(keywords),
