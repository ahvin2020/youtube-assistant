...
from shared.youtube import search_youtube, fetch_channel_recent_videos
...
def enrich_video(vid: dict) -> dict:
    """Add computed fields: duration_category, days_since_upload, is_short."""
...
def main():
    ...
    topics_config = parse_channel_profile(args.channel_profile)

    niche_keywords = topics_config.get("niche_keywords", [])
    own_categories = topics_config.get("own_niche_categories", [])
    adjacent_categories = topics_config.get("adjacent_niche_categories", [])
    all_categories = own_categories + adjacent_categories

    monitored_raw = topics_config.get("monitored_channels", {})
    ...
    own_channel_videos = [enrich_video(v) for v in own_channel_videos]
